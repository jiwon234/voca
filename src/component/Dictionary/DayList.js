            ...
            <ul className="grid grid-cols-2 sm:grid-cols-5 gap-4 m-2 mb-6">
                {days.map(day => ( //days의 각 개체를 day라고 지칭 
                    <li key={day.id} className="h-24 rounded-lg shadow-lg hover:bg-gray-200 duration-100"> {/*key는 있는게 좋다네*/}
                        <Link to={`/day/${day.day}`}
                        className="text-lg flex flex-col items-center justify-center h-full w-full progress">Day {day.day}
                        <div className="mt-3 w-24 bg-gray-200 rounded-full h-1.5 dark:bg-gray-300">
                            <div className="bg-blue-600 h-1.5 rounded-full" style={{width: `${calculateProgress(day.day)}%`}}></div>
                        </div></Link>
                    </li>
                ))}
            </ul>
